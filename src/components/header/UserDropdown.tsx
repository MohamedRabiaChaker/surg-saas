...
import { useUser, authEvents } from "@/context/UserContext";
  ...
  async function onClickSignOut() {
    try {
      ...
      if (res.ok) {
        ...
        // Dispatch logout event to update user context
        authEvents.logout();
        router.push("/signin");
      }
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center">
        <div className="mr-3 h-11 w-11 animate-pulse rounded-full bg-gray-200 dark:bg-gray-700"></div>
        ...
        <div className="pb-3">
          ...
          {user && (
            <div className="mt-2">
              ...
              {user.department && (
                <div className="text-theme-xs mt-1 text-gray-400">
                  {user.department}
                </div>
              )}
              {user.specialization && (
